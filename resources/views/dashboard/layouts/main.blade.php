                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (currUrl == '/dashboard/customers') {
                            ...
                            $.each(data, function(index, value) {
                                row.append("<tr><td><div class='d-flex align-items-center'><img src='/storage/" + value.image + "' alt='' style='width: 45px; height: 45px;' class='rounded-circle'><div class='ms-3'><p class='text-muted mb-0'>" + value.name + "</p></div></div></td><td><p class='text-muted mb-0'>" + value.description + "</p></td><td><p class='text-muted mb-0'>" + value.price + "</p></td><td><a class='badge badge-edit text-white bg-primary rounded-pill d-inline' href='/dashboard/addon/" + value.id + "/edit'>edit</a><form action=/dashboard/addon/" + value.id + "/delete method='POST'>" + method + methodel + "<button type='submit' onclick='return confirm('Are you sure?')' class='badge badge-delete text-white bg-danger rounded-pill d-inline'>delete</button></form></div></div></td></tr>")
                            });
